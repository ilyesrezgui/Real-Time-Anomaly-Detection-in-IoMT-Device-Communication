...
def simulate_iomt_traffic():
    ...
    producer = KafkaProducer( #Creates a producer instance that connects to the Kafka broker.
        bootstrap_servers=[KAFKA_BROKER], #List of broker addresses to connect to.
        value_serializer=json_serializer #Ensures that each message is converted to JSON bytes before sending.
    )
